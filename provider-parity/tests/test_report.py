...
from provider_parity.models import Comparison, Observation, PairAttempt, PairResult
...
def test_report_is_machine_readable_and_self_contained(tmp_path):
    result = PairResult(
        ...
        confirmations=(
            PairAttempt(
                direct=Observation(outcome="success", text="ok"),
                gateway=Observation(outcome="success", text=""),
                comparison=Comparison(verdict="different", differences=("reasoning_presence",)),
            ),
        ),
    )
    ...
    assert payload["results"][0]["confirmations"][0]["comparison"]["verdict"] == "different"
