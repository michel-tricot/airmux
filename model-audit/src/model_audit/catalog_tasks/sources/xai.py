...
from model_audit.catalog_tasks.types import integer, object_list, object_or_empty, required_string, strings
...
class XAI(ModelSource):
    ...
    def fetch(self, key: str | None) -> CatalogValue:
        payload = object_or_empty(self.get(self.url, key))
        ...
        for model in object_list(payload.get("models")):
    ...
    def normalize(self, item: CatalogObject) -> CatalogObject:
        ...
        return self.record(
            required_string(item.get("id"), "xAI model id"),
