...
from model_audit.catalog_tasks.types import boolean, integer, object_list, object_or_empty, required_string, string
...
class Fireworks(ModelSource):
    ...
    def fetch(self, key: str | None) -> CatalogValue:
        collected: list[CatalogObject] = []
        url = self.url
        for _ in range(10):  # 297 models at 200 a page, with headroom
            payload = object_or_empty(self.get(url, key))
            collected.extend(object_list(payload.get("models")))
            token = string(payload.get("nextPageToken"))
            if not token:
                break
    ...
    def normalize(self, item: CatalogObject) -> CatalogObject | None:
        ...
        if boolean(item.get("supportsServerless")) is not True or item.get("state") != "READY":
            return None
        ...
        deprecation = object_or_empty(item.get("deprecationDate"))
        year = integer(deprecation.get("year"))
        month = integer(deprecation.get("month"))
        day = integer(deprecation.get("day"))
        sunset = f"{year:04d}-{month:02d}-{day:02d}" if year is not None and month is not None and day is not None else None
        hf_url = string(item.get("huggingFaceUrl")) or ""
        ...
        return self.record(
            ...
            context_length=integer(item.get("contextLength")),
            input_modalities=["text", "image"] if item.get("supportsImageInput") else ["text"],
            output_modalities=["text"],
            supports_tools=boolean(item.get("supportsTools")),
    ...
    def enrich(self, models: list[CatalogObject]) -> list[CatalogObject]:
        ...
        urls = tuple(
            f"{self.model_docs}{required_string(model.get('id'), 'Fireworks model id')}" for model in enriched if model.get("pricing") is None
        )
