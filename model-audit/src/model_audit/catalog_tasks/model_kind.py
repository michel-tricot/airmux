...
from .types import string, strings
...
def classify(model_id: str, model: CatalogObject | None = None) -> str:
    lowered = (model_id or "").lower()
    for name, pattern in KINDS:
        if re.search(pattern, lowered):
            return name
    outputs = strings((model or {}).get("output_modalities"))
    if outputs and "text" not in outputs:
        return "image" if "image" in outputs else "other"
    return "text"


def text_only(models: list[CatalogObject]) -> list[CatalogObject]:
    """Keep the text models, dropping everything else."""
    return [model for model in models if classify(string(model.get("id")) or "", model) == "text"]
