...
def check_shape(all_entries: list[dict]) -> None:  # noqa: PLR0912 catalog shape validation enumerates independent contract rules
...
def check_schemas(all_entries: list[dict]) -> None:  # noqa: PLR0912 schema validation enumerates independent contract rules
    ...
    for f in sorted((ROOT / "models").glob("*.json")):
        ...
        for m in models:
            ...
            if evidence.get("model_discovery") != expected_discovery:
                fail("models", f"{provider}/{mid} has stale or missing model-discovery parameter evidence; run providers sync --only parameters")
...
def check_seed(all_entries: list[dict]) -> None:
    ...
    for e in all_entries:
        if e["id"] not in seeded:
            fail("seed", f"{e['id']} is in the catalog but not the seed; run providers sync")
            continue
        for field in carried:
            if seeded[e["id"]].get(field) != e.get(field):
                fail("seed", f"{e['id']}.{field} differs between seed and catalog; run providers sync")
