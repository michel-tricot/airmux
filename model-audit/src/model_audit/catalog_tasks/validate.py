...
def fail(failures: list[str], rule: str, detail: str) -> None:
...
def check_shape(all_entries: list[dict], failures: list[str]) -> None:  # noqa: PLR0912 catalog shape validation enumerates independent contract rules
    seen_ids, seen_env = set(), {}
    for e in all_entries:
        eid = e.get("id", "<unnamed>")
        if missing := FIELDS - set(e):
            fail(failures, "fields", f"{eid} is missing {sorted(missing)} from the standard field set")
        if extra := set(e) - FIELDS - PROFILE_FIELDS:
            fail(failures, "fields", f"{eid} has unknown fields {sorted(extra)}")
        if eid in seen_ids:
            fail(failures, "id", f"{eid} is declared twice")
        seen_ids.add(eid)
        ...
        for value in e.get("auth") or []:
            ...
            if value.startswith("header_key:") and len(value.split(":", 1)[1]) < MIN_HEADER_NAME_LENGTH:
                fail(failures, "auth", f"{eid} declares header_key with no header name")
        env = e.get("env_var")
        if not env:
            fail(failures, "env_var", f"{eid} has no credential variable")
        elif env in seen_env and seen_env[env] != eid:
            fail(failures, "env_var", f"{env} is claimed by both {seen_env[env]} and {eid}")
        else:
            seen_env[env] = eid
        if not (e.get("models_url") or "").startswith("http"):
            fail(failures, "models_url", f"{eid} has no usable listing endpoint")
        if not (e.get("base_url") or "").startswith("http"):
            fail(failures, "base_url", f"{eid} has no inference endpoint; the applied taxonomy needs one")


def check_icons(all_entries: list[dict], failures: list[str]) -> None:
    ...
    for e in all_entries:
        for field in ("icon_mono", "icon_color"):
            slug = e.get(field)
            if not slug:
                fail(failures, "icon", f"{e['id']} has no {field}; both variants are required")
                continue
            referenced.add(slug)
            if slug not in available:
                fail(failures, "icon", f"{e['id']}.{field} references {slug}.svg, which is not in taxonomy/icons")
    for orphan in sorted(available - referenced):
        fail(failures, "icon", f"{orphan}.svg is not referenced by any entry")


def check_schemas(all_entries: list[dict], failures: list[str]) -> None:  # noqa: PLR0912 schema validation enumerates independent contract rules
    ...
    for e in all_entries:
        ...
        if not wire:
            if schema is not None:
                fail(failures, "schema", f"{eid} declares schemas but no wire-standard ingress")
            continue
        if not schema:
            fail(failures, "schema", f"{eid} serves {sorted(wire)} but carries no schema")
            continue
        if set(schema) != {"completion"}:
            fail(failures, "schema", f"{eid} has kinds {sorted(schema)}; only completion exists today")
            continue
        if extra := set(schema["completion"]) - wire:
            fail(failures, "schema", f"{eid} has schemas for {sorted(extra)}, which it does not declare as ingress")
        for ingress in sorted(wire - set(schema["completion"])):
            if (eid, ingress) not in KNOWN_GAPS:
                fail(failures, "schema", f"{eid} serves {ingress} but has no schema for it")
        for ingress, parts in schema["completion"].items():
            if bad := set(parts) - PARTS:
                fail(failures, "schema", f"{eid}/{ingress} has unknown parts {sorted(bad)}")
            if "request" not in parts:
                fail(failures, "schema", f"{eid}/{ingress} has no request schema")
            for part, rel in parts.items():
                referenced.add(Path(rel).name)
                path = ROOT / rel
                if not path.exists():
                    fail(failures, "schema", f"{eid}/{ingress}/{part} points at {rel}, which does not exist")
                    continue
                if not Path(rel).name.startswith(f"{ingress}."):
                    fail(failures, "schema", f"{rel} does not start with its ingress {ingress}")
                if not Path(rel).name.endswith(f".{part}.json"):
                    fail(failures, "schema", f"{rel} does not end with its part {part}")
                try:
                    doc = json.loads(path.read_text())
                except json.JSONDecodeError as exc:
                    fail(failures, "schema", f"{rel} is not valid JSON: {exc}")
                    continue
                if not (ROOT_FORMS & set(doc)):
                    fail(failures, "schema", f"{rel} has no root schema form, so it constrains nothing")
                try:
                    Draft202012Validator.check_schema(doc)
                except SchemaError as exc:
                    fail(failures, "schema", f"{rel} is not a valid JSON Schema: {str(exc)[:100]}")

    for f in sorted((ROOT / "schemas" / "completion").glob("*.json")):
        if f.name not in referenced:
            fail(failures, "schema", f"{f.name} is not referenced by any entry")


def check_models(all_entries: list[dict], failures: list[str]) -> None:  # noqa: PLR0912,PLR0915 model validation enumerates independent contract rules
    ...
    for f in sorted((ROOT / "models").glob("*.json")):
        ...
        if provider not in known:
            fail(failures, "models", f"{f.name} is for {provider}, which is in neither catalog")
        if doc.get("source_type") not in {"api", "docs"}:
            fail(failures, "models", f"{provider} has source_type {doc.get('source_type')!r}")
        if doc.get("count") != len(doc.get("models") or []):
            fail(failures, "models", f"{provider} count {doc.get('count')} disagrees with {len(doc.get('models') or [])} entries")
        models = doc.get("models") or []
        if [m.get("id") for m in models] != [m.get("id") for m in sort_models([dict(m) for m in models])]:
            fail(failures, "models", f"{provider} is not sorted by id; regenerate so diffs stay reviewable")
        ...
        for m in models:
            ...
            if leading != [k for k in MODEL_ORDER if k in m]:
                fail(failures, "models", f"{provider}/{m.get('id')} has non-canonical key order")
                break
        seen = set()
        for m in models:
            mid = m.get("id")
            if not mid:
                fail(failures, "models", f"{provider} has a model with no id")
                continue
            if mid in seen:
                fail(failures, "models", f"{provider} lists {mid} twice")
            seen.add(mid)
            if (kind := classify(mid, m)) != "text":
                fail(failures, "models", f"{provider}/{mid} classifies as {kind}; the catalog is text-only")
            for field in ("input_modalities", "output_modalities"):
                ...
                if not isinstance(modalities, list) or not modalities or any(not isinstance(modality, str) for modality in modalities):
                    fail(failures, "models", f"{provider}/{mid}.{field} must be a non-empty list of canonical modalities")
                    continue
                ...
                if unknown_modalities := sorted(set(typed_modalities) - set(MODALITIES)):
                    fail(failures, "models", f"{provider}/{mid}.{field} uses unknown modalities {unknown_modalities}")
                if len(typed_modalities) != len(set(typed_modalities)):
                    fail(failures, "models", f"{provider}/{mid}.{field} repeats a modality")
            context_source = m.get("context_source")
            output_source = m.get("max_output_source")
            if not known_source(context_source, VALUE_SOURCE):
                fail(failures, "models", f"{provider}/{mid} has context_source {context_source!r}")
            if not known_source(output_source, VALUE_SOURCE):
                fail(failures, "models", f"{provider}/{mid} has max_output_source {output_source!r}")
            if bool(m.get("context_length")) != bool(context_source):
                fail(failures, "models", f"{provider}/{mid} must carry context_length and context_source together")
            if bool(m.get("max_output_tokens")) != bool(output_source):
                fail(failures, "models", f"{provider}/{mid} must carry max_output_tokens and max_output_source together")
            psource = m.get("pricing_source")
            if not known_source(psource, VALUE_SOURCE):
                fail(failures, "models", f"{provider}/{mid} has pricing_source {psource!r}")
            if psource and not m.get("pricing"):
                fail(failures, "models", f"{provider}/{mid} claims a pricing source but carries no price")
            if m.get("pricing") and not psource:
                fail(failures, "models", f"{provider}/{mid} has a price with no pricing_source")
            if "vendor-docs" in {context_source, output_source, psource} and not str(m.get("documentation_url", "")).startswith("https://"):
                fail(failures, "models", f"{provider}/{mid} cites vendor-docs without an HTTPS documentation_url")
            if m.get("source_conflicts") and not str(m.get("documentation_url", "")).startswith("https://"):
                fail(failures, "models", f"{provider}/{mid} records a source conflict without an HTTPS documentation_url")
            evidence = m.get("parameter_evidence") or {}
            if extra_sources := set(evidence) - {"model_discovery"}:
                fail(failures, "models", f"{provider}/{mid} has unknown parameter evidence {sorted(extra_sources)}")
            if evidence.get("model_discovery") != expected_discovery:
                fail(
                    failures,
                    "models",
                    f"{provider}/{mid} has stale or missing model-discovery parameter evidence; run providers sync --only parameters",
                )
            if not evidence.get("model_discovery", {}).get("sources"):
                fail(failures, "models", f"{provider}/{mid} has model-discovery parameter evidence without a request schema")


CANDIDATE_FIELDS = {"id", "name", "homepage", "docs", "env_var"}


def check_candidates(all_entries: list[dict], failures: list[str]) -> None:
    ...
    for entry in yaml.safe_load(path.read_text())["candidates"]:
        ...
        if set(entry) != CANDIDATE_FIELDS:
            fail(failures, "candidate", f"{eid} has {sorted(set(entry) ^ CANDIDATE_FIELDS)}; candidates are identity only")
        if eid in active:
            fail(failures, "candidate", f"{eid} is both active and a candidate")
        if (ROOT / "models" / f"{eid}.json").exists():
            fail(failures, "candidate", f"{eid} still has a model catalog; candidates carry no derived data")


def check_applied(failures: list[str]) -> None:
    ...
    if changed:
        fail(failures, "applied", "taxonomy.yml is out of date; run airmux-audit taxonomy build")


def check_seed(all_entries: list[dict], failures: list[str]) -> None:
    ...
    for e in all_entries:
        if e["id"] not in seeded:
            fail(failures, "seed", f"{e['id']} is in the catalog but not the seed; run providers sync")
            continue
        for field in carried:
            if seeded[e["id"]].get(field) != e.get(field):
                fail(failures, "seed", f"{e['id']}.{field} differs between seed and catalog; run providers sync")
    for extra in sorted(set(seeded) - {e["id"] for e in all_entries}):
        fail(failures, "seed", f"{extra} is in the seed but not the catalog")


def run() -> CatalogValidated | ValidationFailed:
    failures: list[str] = []
    all_entries = entries()
    check_shape(all_entries, failures)
    check_icons(all_entries, failures)
    check_schemas(all_entries, failures)
    check_models(all_entries, failures)
    check_candidates(all_entries, failures)
    check_applied(failures)
    check_seed(all_entries, failures)

    if failures:
        return ValidationFailed(tuple(failures))
    ...
    return CatalogValidated(
        len(all_entries),
        len(list((ROOT / "schemas" / "completion").glob("*.json"))),
        len(list((ROOT / "icons").glob("*.svg"))),
        len(models),
        complete,
    )
