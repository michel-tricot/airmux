...
def load_models_dev(providers: list[str]) -> tuple[dict[tuple[str, str], CatalogObject], tuple[tuple[str, str], ...]]:
    ...
    for ours in providers:
        ...
        for model_id, value in models.items():
            ...
            cost = object_or_empty(model.get("cost"))
            limit = object_or_empty(model.get("limit"))
...
def load_openrouter() -> dict[str, CatalogObject]:
    ...
    table: dict[str, CatalogObject] = {}
    for model in object_list(object_or_empty(get(OPENROUTER)).get("data")):
        cost = object_or_empty(model.get("pricing"))
        top = object_or_empty(model.get("top_provider"))
        ...
        for value in (model.get("id"), model.get("canonical_slug"), model.get("hugging_face_id")):
            if key := string(value):
                table.setdefault(norm(key), record)
    return table


def apply(model: CatalogObject, candidate: CatalogObject, source: str, counts: dict[str, int]) -> None:
...
def fill_from_aliases(models: list[CatalogObject], counts: dict[str, int]) -> None:
    ...
    for model in models:
        for name in strings(model.get("aliases")):
