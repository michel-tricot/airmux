...
1. Run `uv run airmux-audit cases coverage` and `uv run airmux-audit taxonomy validate`
...
7. Run `uv run airmux-audit providers onboard <provider>`
...
2. Run `uv run airmux-audit providers sync <provider>`
...
5. Run `uv run airmux-audit taxonomy validate`
...
Prefer `uv run airmux-audit providers sync <provider> --only models` when the provider lists
the model. If the model is absent from the listing, verify it against a current vendor-owned
source and run `uv run airmux-audit models add <provider> <model> --source <url>`. Supply both
...
1. Inspect the plan with `uv run airmux-audit runs plan`
2. Execute raw HTTP comparisons against an already running gateway with `uv run airmux-audit runs execute`
...
6. Review the report before running `uv run airmux-audit evidence accept <report>`
...
Run `uv run airmux-audit taxonomy build`, then `uv run airmux-audit cases coverage` and
`uv run airmux-audit taxonomy validate`. Review `taxonomy/behavior.json` and
`taxonomy/taxonomy.yml` as generated projections; never edit them directly.

For a clean source audit, run `uv run airmux-audit taxonomy rebuild --preserve-as <name>`.
Compare every field with `uv run airmux-audit taxonomy diff <name> taxonomy`; use
