...
from model_audit.surfaces.base import parse_sse
...
def test_sse_parser_preserves_named_events_as_semantic_types():
    events = parse_sse('event: message_stop\ndata: {"message":"done"}\n\n')

    assert events[0].event == "message_stop"
    assert events[0].payload == {"type": "message_stop", "message": "done"}
...
def test_responses_stream_error_event_preserves_the_gateway_error():
    event = {"type": "error", "error": {"code": "invalid_upstream_response", "message": "provider stream ended early"}}
    ...
    assert observation.outcome == "error"
    assert observation.error_code == "invalid_upstream_response"
    assert observation.error_message == "provider stream ended early"
