...
[protect-main.json](protect-main.json) requires pull requests, squash merges, linear history, an up-to-date base,
...
The policy uses strict current-base checks and does not configure a merge queue. Reassess merge strategy separately
when repository capabilities or collaboration needs change.
...
Repository administrators have a `pull_request` bypass for exceptional merges, such as unavailable required checks or
current-base checks that cannot settle during concurrent merges. It does not permit direct pushes, branch deletion,
