...
| 400 | `ambiguous_credentials` | A credential header is repeated or bearer and API-key headers contain different keys |
...
| 401 | `missing_bearer_token` | No bearer token, API-key header, or valid playground cookie |
| 401 | `invalid_token` | Authorization is malformed or the inference key is unknown, expired, revoked, or not adopted yet |
