...
Explicit header credentials take precedence over the playground cookie. Repeated credential headers or different keys
in the two headers return `400 ambiguous_credentials`; matching keys are accepted. Malformed authorization returns
`401 invalid_token` without falling back to the cookie. Cookie authentication requires `X-Requested-With`; when
...
| `WWW-Authenticate` | `Bearer realm="tokkeeper"` | `401` responses |
