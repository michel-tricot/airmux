...
Browser sessions use the `tokkeeper_session` cookie and must also send `X-Requested-With`. Inference keys are not accepted by management endpoints.
...
curl https://llm.example.com/api/v1/organizations/$ORG_ID/workspaces \
  -H "Authorization: Bearer $TOKKEEPER_MANAGEMENT_KEY" \
