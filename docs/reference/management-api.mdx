...
Bounded collections, such as a workspace's policies and rules, remain `{"data": [...]}`. Errors retain FastAPI's
`{"detail": ...}` shape and are not enveloped.
...
Growing collection endpoints use cursor pagination:
...
Do not decode, edit, or reuse a cursor with another endpoint, tenant, scope, or filter. Invalid, oversized, stale-version,
and mismatched cursors return `422` with `{"detail":"invalid cursor"}`. Cursors identify a position, not a database row,
so traversal can continue if the item at the previous page boundary is deleted.
