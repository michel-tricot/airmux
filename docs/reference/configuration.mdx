...
Budget configuration is independent of the bundle source and event outbox. With `budget.kind` set to `none`, matching
budget requests fail closed with `503 policy_state_unavailable` instead of serving without budget state.
