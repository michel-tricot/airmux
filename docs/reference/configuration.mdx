...
Budget configuration is independent of the bundle source and event outbox. A bundle containing budget rules is rejected
when `budget.kind` is `none`, so standalone gateways fail closed instead of serving requests without budget state.
