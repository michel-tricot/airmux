...
SQLite flush defaults to 5 seconds. Its cache directory defaults to `.airmux` beside the configuration file, and the
outbox writes `events.db` there. Workers sharing one gateway cache directory coordinate through the same outbox.
