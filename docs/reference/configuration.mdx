...
`./airmux-config/airmux.yml` is mounted through the Compose file. See [Docker customization](/docs/deployment/customization).
...
| `AIRMUX_IMAGE`            | `airmux:local`          | Image used by every role in either topology; set to one published release in `.env` |
