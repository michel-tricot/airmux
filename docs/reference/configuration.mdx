...
Budget configuration is independent of the bundle source and event outbox. Missing, mismatched, or expired state allows
requests. With `budget.kind` set to `none`, budget rules always allow requests.
