...
Capabilities are `streaming`, `tools`, `reasoning`, and `structured_output`. AirLLM derives requirements from the
...
```bash
airllm taxonomy apply --file taxonomy/taxonomy.yml --dry-run
airllm taxonomy apply --file taxonomy/taxonomy.yml
```
