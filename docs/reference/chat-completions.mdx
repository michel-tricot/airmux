...
description: "Reference the Chat Completions-compatible request, response, streaming, and dialect-selection behavior."
---
...
This route accepts Chat Completions or canonical JSON. Official SDK user-agents and unambiguous OpenAI fields such as
`max_completion_tokens` select Chat Completions automatically. Set `x-tokkeeper-dialect: openai_native` only for raw
requests with neither signal when you need a Chat Completions-shaped response.
...
| `max_tokens` | integer | Canonical name; minimum 1 |
| `max_completion_tokens` | integer | Chat Completions alias for `max_tokens` |
