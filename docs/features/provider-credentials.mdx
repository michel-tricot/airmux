...
Provider credentials are upstream API keys that TokKeeper uses after an inference request has been authenticated and
...
```bash
printf '%s' "$OPENAI_API_KEY" | tokkeeper provider-credentials add openai --name production --priority 10
```
...
```bash
tokkeeper provider-credentials list
tokkeeper provider-credentials list --org
```
...
```bash
printf '%s' "$OPENAI_API_KEY_NEXT" | tokkeeper provider-credentials rotate "$CREDENTIAL_ID"
tokkeeper provider-credentials disable "$CREDENTIAL_ID"
tokkeeper provider-credentials disable "$CREDENTIAL_ID" --enable
tokkeeper provider-credentials rm "$CREDENTIAL_ID"
```
