...
One `request_id` is one logical request, even when fallback creates several provider attempts. Recorded cost and tokens add across attempts, including failed attempts with
