...
airmux records one usage event for each upstream attempt, including failed and cancelled attempts. Before provider spend,
the gateway reserves enough bounded queue capacity for every possible attempt. It persists events locally on a dedicated
storage thread and exports durable events to the control plane in batches.
