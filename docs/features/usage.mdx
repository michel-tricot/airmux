...
The console's Usage and Requests pages are available at organization and workspace scope. They use the same report
...
details, filter choices, and exports.
...
The Requests page supports exact request ID lookup, pagination, and CSV export of the complete filtered result. Detail
...
The shared API includes `/usage`, `/attribution`, `/filter-options`, `/requests`, `/requests/{request_id}`, and
`/requests/export`. Periods include Today, 7 days, 30 days, month to date, and custom dates. Reports compare with the
preceding interval of equal duration. Request lists and attribution use `limit` and `offset`.
