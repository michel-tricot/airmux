...
Every inference request names one catalog model. AirLLM resolves that model to a provider profile, validates the
...
AirLLM checks capabilities and modalities before it resolves a provider credential.
...
AirLLM uses the first populated scope allowed by policy. Within that scope, lower numeric priority is tried first and
...
Provider profiles define aliases, accepted extra fields, and model-specific parameter support. Before the upstream call, AirLLM may:
...
<Info>
  Fallback is not exactly-once generation. A timed-out provider may continue work after AirLLM starts another attempt,
