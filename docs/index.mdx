...
`airmux` is a self-hosted LLM gateway. Agents send requests using the `Chat Completions`, `Responses`, or `Messages` API. 

`airmux` handles provider's authentication, applies policy, makes the request to the model and provider, records usage, and gives you a persistent usage history with cost trakcing.
