...
1. On `main`, run **Actions → Prepare Release** and choose patch, minor, or major
2. Open the pull request link in the workflow summary. Confirm that only `VERSION` changed, then merge after PR CI and Security pass
3. Wait for **Main CI** and **Security** to pass on the version commit. Main CI includes gateway, full-stack, browser, and Docker validation
4. Run **Actions → Publish Release** from `main`. Confirm its `prepare`, `installation`, and `live-providers` jobs pass before the tag and publication jobs start
5. Confirm the `vX.Y.Z` tag and GitHub release point to that commit, PyPI contains the package, and GHCR `X.Y.Z` and `latest` resolve to the verified image digest

The workflow uses the Python and container candidates from that exact Main CI run. The `release` environment supplies provider credentials and `RELEASE_GITHUB_TOKEN`; PyPI uses its trusted publisher for `release.yml`.
