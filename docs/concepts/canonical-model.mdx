...
AirLLM uses a canonical model as the waist between callers and providers. Each ingress dialect translates into canonical
...
1. `x-airllm-dialect` override
2. Unambiguous request shape or SDK user-agent
3. Canonical as the default

For raw text-only Chat Completions requests, set `x-airllm-dialect: openai_native`. Official SDK requests are recognized automatically.
...
errors even when AirLLM routed the request to a different provider family.
