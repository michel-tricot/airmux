...
The image accepts five roles: `setup`, `control-plane`, `data-plane`, `console`, and `all-in-one`. Setup is finite and
owns bootstrap-key creation, forward migrations, and shipped-taxonomy application. The three serving roles perform no
deployment initialization. The default all-in-one role runs setup, then supervises one process for each serving role.

The all-in-one topology uses one application container and Postgres. The split topology projects the same image digest
