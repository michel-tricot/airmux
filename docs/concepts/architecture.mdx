...
The image accepts four roles: `control-plane`, `data-plane`, `console`, and `airmux`. The control plane owns bootstrap-key
creation, forward migrations, and shipped-taxonomy application. The default `airmux` role supervises one process for each
serving role and holds the gateway until the control plane is healthy.
