...
<Steps>
  <Step title="Authenticate">
    The gateway loads its current bundle set, hashes the bearer token, and looks up the active inference-key entry.
    Missing, unknown, expired, or not-yet-adopted keys are rejected.
  </Step>

  <Step title="Parse">
    The route or dialect resolver chooses an ingress adapter. The adapter validates the caller's wire shape and produces
  ...
  <Step title="Account">
    TokKeeper records tokens, cache usage, estimated status, cost, latency, route, credential metadata, bundle ID, and
    outcome. The local outbox exports the event to the control plane.
  </Step>
</Steps>
