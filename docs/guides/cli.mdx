...
<Note>
  The [Docker quickstart](/docs/quickstart) includes the CLI in the image. 

  From its Compose directory, prefix commands with `docker compose exec app`, for example `docker compose exec app airmux doctor`.
</Note>
