---
title: "Separate services and scale gateways"
description: "Run the console, control plane, and multiple data planes independently with distinct persistent state."
---

The `docker-compose.split.yml` example runs Postgres, one control plane, the console proxy, a readiness-aware inference router,
and two gateways on one Docker host.

```bash
docker compose -p tokkeeper-split -f docker-compose.split.yml up -d --build --wait
uv run --package cli --no-dev --frozen tokkeeper quickstart --url http://localhost:8080
```

The inference router distributes `/inf/` requests only to gateways whose `/readyz` check succeeds. A gateway remains out of
rotation while it has no bundle or has rejected the latest manifest. `/healthz` remains the process liveness check.

## Volume ownership

| Volume | Owner |
| --- | --- |
| `split-pgdata` | Postgres |
| `runtime-credentials` | Control plane writes; gateways read |
| `provider-secrets` | Control plane writes; gateways read |
| `gateway-1`, `gateway-2` | One gateway each; identity, bundle cache, usage outbox |

Every additional gateway needs a new persistent state volume and a server entry in `deploy/docker/haproxy.cfg`. Sharing one
SQLite outbox or cloning a running gateway identity is invalid.

## Multi-host requirements

The checked-in split file demonstrates service boundaries, not a multi-host production topology. Across hosts, provide:

- Reachable Postgres for the control plane
- One secret store accessible to both planes, with the control plane able to write and data planes able to read
- A private authenticated control-plane URL for bundle polling, event ingestion, and heartbeat
- One persistent cache and outbox directory per gateway
- A streaming-capable load balancer with response buffering disabled

Set `TOKKEEPER_DATAPLANE_CONTROL_PLANE_URL` to the private control-plane address. Protect this path and its data-plane management key.

Keep one control-plane replica with the checked-in startup process. Multiple control-plane replicas require coordinated
migration and catalog initialization plus shared credential storage.

## Migrate from all-in-one

The default and split Compose files use different named volumes. Migration requires a database dump plus a stopped copy
of `/state/runtime`, `/state/secrets`, and one gateway's `/state/data-plane`. Restore each into the corresponding split
volume before the first split startup.

<Warning>
  Do not start the split stack against a partial copy. Database metadata and provider secrets must move together.
</Warning>
