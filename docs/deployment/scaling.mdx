...
docker compose -p airmux-split -f docker-compose.split.yml up -d
...
The same optional [Docker configuration directory](/docs/deployment/docker#customize-the-runtime-yaml) is mounted in every role.
...
| `runtime-credentials` | Control plane writes and gateways read |
| `provider-secrets` | Control plane writes and gateways read |
| `gateway-1`, `gateway-2` | One gateway each: identity, bundle cache, usage outbox |

Every additional gateway needs a new persistent state volume.
...
- One secret store accessible to both planes, with the control plane able to write and data planes able to read (until we support vaults, we recommend that you use `insecure_database`)
...
Keep one control-plane replica. Multiple control-plane replicas isn't supported right now.
...
Use one release digest for every role. Read its release notes, compare any
[custom runtime YAML](/docs/deployment/docker#customize-the-runtime-yaml) with the new image, and back up Postgres,
`runtime-credentials`, `provider-secrets`, and every gateway state volume together. A database migration may prevent
the previous image from starting, so record the current digest and confirm the backup can be restored.

Remove gateways from public traffic and let active streams finish. Use the target release's split and base Compose
files, set `AIRMUX_IMAGE` to the target digest, then run:

```bash
docker compose -p airmux-split -f docker-compose.split.yml up -d --wait
```

Wait for the control plane and both gateways to report ready, then restore traffic and check a buffered request, a
streaming request, and usage export. If verification fails after migration, stop the new stack, restore Postgres and
all state volumes from the same recovery point, and redeploy the previous digest. Do not run the previous image on
