...
Every service and one-shot job references `AIRMUX_IMAGE`, so pin that variable to one published digest for coordinated
deployments. The migration job updates Postgres, the taxonomy job applies the shipped catalog, and only then does the control
plane start. The control plane creates the shared bootstrap credential and verifies the schema before becoming healthy. Both
gateways start after that health check succeeds. The console proxy distributes `/inf/` requests between them.
