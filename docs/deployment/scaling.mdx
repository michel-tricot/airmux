...
Every service references `AIRMUX_IMAGE`, so pin that variable to one published digest for coordinated deployments. The
control plane creates the shared bootstrap credential, migrates Postgres, and applies the shipped taxonomy before becoming
healthy. Both gateways start after that health check succeeds. The console proxy distributes `/inf/` requests between them.
