...
The default Compose port mapping listens on all host interfaces. For local bootstrap, bind it to loopback and claim the
first owner before exposing the service:

```bash
AIRMUX_PORT=127.0.0.1:8080 docker compose up -d --wait
```

If you choose another port, update `AIRMUX_PUBLIC_URL` and the CLI URL to match. For a remote bootstrap, firewall the
published port until the owner claim is complete.
