...
2. Compare a [custom runtime YAML](/docs/deployment/customization), if present, with the target image's bundled default
3. Record the current release or image digest
4. Stop new inference traffic and allow active streams to finish
5. Back up Postgres, runtime credentials, provider secrets, and every gateway state directory together
6. Confirm that the database backup and state snapshot can be restored on isolated infrastructure
