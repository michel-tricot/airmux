...
Replace an airmux container as one unit. Compose completes the migration and taxonomy jobs, then the gateway becomes ready.

For a split deployment:

1. Remove every gateway from public traffic and stop it after active streams finish
2. Run the migration and taxonomy jobs, then replace the control plane, gateways, and console with the same image digest
