...
<Warning>
  Control-plane and all-in-one containers run the forward database migration during startup. The CLI has no operator-facing downgrade command. Do not
  run an older release against a database migrated by a newer release.
</Warning>
...
Replace an all-in-one container as one unit. On startup, it migrates the database, applies the shipped catalog, and then
starts the control plane, gateway, and console.
