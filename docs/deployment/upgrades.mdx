...
<Warning>
  Control-plane startup runs the forward database migration. The CLI has no operator-facing downgrade command. Do not run an
...
Replace an airmux container as one unit. The control plane initializes, then the gateway becomes ready.

For a split deployment:

1. Remove every gateway from public traffic and stop it after active streams finish
2. Replace the control plane, gateways, and console with the same image digest
