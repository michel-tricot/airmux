...
| `AIRLLM_PUBLIC_SIGNUP` | Set to `false` to require an invitation after the instance is claimed |
...
After claiming the instance, set `AIRLLM_PUBLIC_SIGNUP=false` and restart the application to close
public account creation. The sign-in page then directs new users to contact an instance
administrator. Valid organization invitation links can still create an account for their invited
email address. This setting never prevents the first account from claiming an unclaimed instance.
