...
Restarting the airmux container by itself does not run either job. The control plane never changes the database schema or
catalog while starting; it refuses to serve when the schema is out of date and tells you to run the migration command.
...
starts the control plane, gateway, and console. The control plane verifies the migrated schema and creates its internal
