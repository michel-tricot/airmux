...
The default `docker-compose.yml` runs one long-lived airmux container and Postgres. Two one-shot containers migrate the
database and apply the shipped taxonomy before airmux starts.
...
The migration and taxonomy jobs use the same image as the serving container and must complete successfully before it starts.
The image defaults to the `airmux` role, which starts the control plane, gateway, and console. The control plane verifies
the migrated schema and creates its configured bootstrap key only when absent. Application processes run as UID `10001`.
