...
description: "Run the all-in-one AirLLM image and Postgres with persistent named volumes."
---

The default `docker-compose.yml` runs two containers: AirLLM and Postgres.
...
reverse proxy and set `AIRLLM_PUBLIC_URL` to the exact HTTPS origin.
...
AIRLLM_PORT=18080 AIRLLM_PUBLIC_URL=http://localhost:18080 \
  DEPLOYMENT_PROJECT=airllm-test \
