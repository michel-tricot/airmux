...
description: "Run the all-in-one airmux image and Postgres with persistent named volumes."
---
...
The image defaults to the `all-in-one` role. That role runs the same finite `setup` operation used by split deployments,
