...
The image defaults to the `airmux` role, which starts the control plane, gateway, and console. The control plane creates
its configured bootstrap key only when absent, applies forward migrations and the shipped taxonomy, then accepts traffic.
Application processes run as UID `10001`.
