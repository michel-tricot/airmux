...
The all-in-one image applies database migrations and the shipped catalog before it accepts traffic. A split gateway can
keep serving an accepted cached bundle while the control plane is unavailable. Because pre-1.0 releases do not promise
cross-version compatibility between planes, deploy one release across all components. Follow
