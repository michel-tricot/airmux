...
`/healthz` reports whether each process can perform its serving role. Use it for traffic admission and health monitoring.

Compose probes check that the management API and gateway HTTP listeners are available so `up --wait` can finish before onboarding creates the first bundle.

| Plane | Endpoint | Healthy response | `503` meaning |
| --- | --- | --- | --- |
| Control plane | `/healthz` | `200 {"status":"ready"}` | Postgres is unavailable |
| Data plane | `/healthz` | `200 {"status":"ready"}` | No accepted bundle snapshot is available, or the metering writer cannot accept work |
| Combined listener | `/healthz` | `200 {"status":"ready"}` | The control plane or data plane cannot perform its role |

The combined listener checks both planes. A data plane that rejects a newer manifest keeps serving its last accepted
snapshot and remains healthy, even during a control-plane outage. The combined listener reports that outage as unhealthy.
...
| Data-plane `/healthz` returns `503` | Bundle poll authentication, control-plane reachability, and cached state mount |
