...
| Data plane | `/readyz` | `200 {"status":"ready"}` | No accepted bundle snapshot is available |
...
all-in-one public listener does not proxy this path. Scrape the internal listeners from a monitoring network instead:
...
| `airmux_data_plane_metering_shutdown_drains_total` | Graceful shutdown drain outcomes |
...
- failed metering exports or shutdown drains
