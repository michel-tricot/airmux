...
To restrict a principal across their credentials in this workspace, replace the target with `{"kind": "selected_users", "user_ids": ["USER_UUID"]}`. Use `{"kind": "selected_keys", "key_ids": ["KEY_ID"]}` for specific credentials. Key inspection lists all applicable policies.
