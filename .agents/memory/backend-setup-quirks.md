...
## taxonomy --file path
`uv run tokkeepercp taxonomy` defaults to looking for `taxonomy.yml` next to `tokkeeper.yml` (project root). The actual file lives at `taxonomy/taxonomy.yml`. The backend script (`scripts/replit-backend.sh`) must pass `--file taxonomy/taxonomy.yml`.
...
**How to apply:** `uv run tokkeepercp taxonomy --file taxonomy/taxonomy.yml` in `scripts/replit-backend.sh`.
...
Run the backend via `scripts/replit-backend.sh` (not raw `uv run tokkeepercp migrate`) — the script normalizes `sslmode` → `ssl` in the DATABASE_URL for asyncpg compatibility.

## Signing keys
`uv run tokkeepercp keygen` writes `.tokkeeper/signing.key` and `.tokkeeper/signing.pub`. These are file-based secrets read by `tokkeeper.yml` directly — not environment variables. The backend script runs keygen if they don't exist.
