...
from control_plane.models.policy import Policy, PolicyCreate, PolicyOut, PolicyUpdate
...
async def create_policy(workspace: WorkspaceDep, body: PolicyCreate) -> Envelope[PolicyOut]:
    ...
    policy = Policy(org_id=workspace.org_id, workspace_id=workspace.id, name=body.name, enabled=body.enabled, definition=body.definition)
    await policy.save()
