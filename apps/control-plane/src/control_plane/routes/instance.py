...
@router.post("/orgs")
async def create_org(body: OrgIn) -> Envelope[OrgOut]:
    if await Org.get(body.id) is not None:
        raise HTTPException(status_code=409)
    await Org(id=body.id, name=body.name or body.id).save()
    return Envelope(data=OrgOut(id=body.id))


@router.get("/orgs")
async def list_orgs() -> Envelope[list[Org]]:
    return Envelope(data=await Org.find(order_by=col(Org.id)))


@router.post("/users")
async def create_user(body: UserIn) -> Envelope[UserOut]:
...
async def remove_membership(user_id: str, org_id: str) -> Envelope[MembershipOut]:
    membership = await OrgMembership.get((user_id, org_id))
    if membership is None:
        raise HTTPException(status_code=404)
    await membership.delete()
    return Envelope(data=MembershipOut(user_id=user_id, org_id=org_id, status="removed"))


@router.post("/users/{user_id}/tokens")
async def mint_user_token(user_id: str, request: Request, body: UserTokenIn | None = None) -> Envelope[MgmtTokenOut]:
...
@router.post("/tokens")
async def mint_instance_token(request: Request) -> Envelope[MgmtTokenOut]:
    return Envelope(data=await _mint(request, None))


@router.post("/orgs/{org_id}/tokens")
async def mint_org_token(org_id: str, request: Request) -> Envelope[MgmtTokenOut]:
    if await Org.get(org_id) is None:
        raise HTTPException(status_code=404)
...
@router.get("/tokens")
async def list_tokens(org_id: str | None = None) -> Envelope[list[MgmtToken]]:
    conditions = (MgmtToken.org_id == org_id,) if org_id else ()
    return Envelope(data=await MgmtToken.find(*conditions, order_by=col(MgmtToken.id)))
