...
@router.get("")
async def get_taxonomy(_claims: MgmtDep) -> TaxonomyOut:
...
async def create_provider(_claims: InstanceDep, body: ProviderIn) -> ProviderOut:
...
@router.post("/models")
async def create_model(_claims: InstanceDep, body: ModelIn) -> ModelOut:
    """Create or update: reapplying a taxonomy converges the catalog."""
    try:
