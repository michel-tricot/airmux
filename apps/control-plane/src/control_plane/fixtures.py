...
unacceptable outside development. `tokkeepercp fixtures` refuses any database that already holds
human accounts.
...
class MissingProvidersError(ValueError):
    ...
    def __init__(self, missing: list[str]) -> None:
        super().__init__(f"the catalog has no {', '.join(missing)}; run `tokkeepercp taxonomy` to fill it before seeding")
