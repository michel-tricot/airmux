...
from sqlalchemy import JSON, ForeignKeyConstraint, Index, TypeDecorator, text
...
class Policy(Record, Identified, OrgOwned, Tombstonable, table=True):
    __table_args__: ClassVar = (
        ...
        Index("policy_workspace_id_idx", "workspace_id", "id"),
    ...
    async def for_workspace(cls, workspace_id: UUID) -> list[Self]:
        return await cls.find(cls.workspace_id == workspace_id, order_by=(col(cls.id),))
    ...
    def entry(self) -> PolicyEntry:
        return PolicyEntry(id=self.id, workspace_id=self.workspace_id, name=self.name, definition=self.definition)
