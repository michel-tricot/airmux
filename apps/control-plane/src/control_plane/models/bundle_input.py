...
def bundle_input[T: Record](*, scope: BundleInputScope, ignored_columns: tuple[str, ...] = ()) -> Callable[[type[T]], type[T]]:
    def register(cls: type[T]) -> type[T]:
        _BUNDLE_INPUTS[cls] = BundleInput(model=cls, scope=scope, ignored_columns=ignored_columns)
        return cls
...
def bundle_input_trigger_ddl_v2(table: str, scope: BundleInputScope, ignored_columns: tuple[str, ...] = ()) -> tuple[str, str]:
    function = (
        "CREATE OR REPLACE FUNCTION mark_bundle_stale_v2() RETURNS trigger LANGUAGE plpgsql AS $$ "
        "DECLARE position integer; old_value jsonb; new_value jsonb; old_org uuid; new_org uuid; BEGIN "
        "IF TG_OP = 'UPDATE' THEN old_value := to_jsonb(OLD) - ARRAY['created_at', 'updated_at']; "
        "new_value := to_jsonb(NEW) - ARRAY['created_at', 'updated_at']; "
        "IF TG_NARGS > 1 THEN FOR position IN 1..TG_NARGS - 1 LOOP "
        "old_value := old_value - TG_ARGV[position]; new_value := new_value - TG_ARGV[position]; END LOOP; END IF; "
        "IF old_value IS NOT DISTINCT FROM new_value THEN RETURN NULL; END IF; END IF; "
        "IF TG_ARGV[0] = 'global' THEN "
        "INSERT INTO global_bundle_state (id, desired_generation) VALUES (1, 1) ON CONFLICT (id) DO UPDATE "
        "SET desired_generation = global_bundle_state.desired_generation + 1; RETURN NULL; END IF; "
        "IF TG_OP <> 'INSERT' THEN old_org := NULLIF(to_jsonb(OLD) ->> 'org_id', '')::uuid; END IF; "
    ...
    arguments = ", ".join(f"'{value}'" for value in (scope, *ignored_columns))
    trigger = (
        f'CREATE OR REPLACE TRIGGER {table}_bundle_stale AFTER INSERT OR UPDATE OR DELETE ON "{table}" '
        f"FOR EACH ROW EXECUTE FUNCTION mark_bundle_stale_v2({arguments})"
    )
    return function, trigger


def bundle_input_trigger_drop_ddl_v2(table: str) -> tuple[str, str]:
    drop_trigger = f'DROP TRIGGER IF EXISTS {table}_bundle_stale ON "{table}"'
    drop_function = (
        "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_trigger t JOIN pg_proc p ON t.tgfoid = p.oid "
        "WHERE p.proname = 'mark_bundle_stale_v2' AND NOT t.tgisinternal) "
        "THEN DROP FUNCTION IF EXISTS mark_bundle_stale_v2(); END IF; END $$"
    )
    return drop_trigger, drop_function
