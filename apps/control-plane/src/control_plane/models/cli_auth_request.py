...
class CliAuthRequest(Record, Identified, Tombstonable, table=True):
    ...
    Only hashes of both are stored; open() is the only place the plaintexts exist. The management
    key is minted at poll time, after approval, so its plaintext never rests in the pending request;
    ...
    approved_user_id: UUID | None = Field(default=None, foreign_key="user.id")
    approved_org_id: UUID | None = Field(default=None, foreign_key="org.id")
    ...
    async def open(cls, client_name: str, requester: str) -> tuple[Self, str, str]:
        ...
        auth_request = await cls(
            ...
            requester=requester,
            expires_at=datetime.now(tz=UTC) + AUTH_REQUEST_TTL,
        ).save()
    ...
    @classmethod
    async def by_poll_secret(cls, secret: str) -> Self | None:
        if not secret.startswith(POLL_SECRET_PREFIX):
            return None
        return await cls.first(cls.poll_secret_hash == token_hash(secret))
