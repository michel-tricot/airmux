...
def test_access_key_api_mints_lists_and_revokes_one_resource_type(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        listed = client.get(f"/api/v1/orgs/{org_id}/access-keys", headers=root)
        ...
        revoked = client.delete(f"/api/v1/access-keys/{key['id']}", headers=root)
        assert revoked.status_code == 200, revoked.text
        assert revoked.json()["data"]["status"] == "revoked"
        assert client.get(f"/api/v1/orgs/{org_id}/workspaces", headers=child_headers).status_code == 401
        relisted = client.get(f"/api/v1/orgs/{org_id}/access-keys", headers=root).json()["data"]
...
def test_access_key_permissions_are_required_and_validated(tmp_path):
    ...
    with TestClient(cp.app) as client:
        headers = cp.headers()
        assert client.post("/api/v1/instance/access-keys", json={"label": "missing"}, headers=headers).status_code == 422
        assert client.post("/api/v1/instance/access-keys", json={"label": "empty", "permissions": []}, headers=headers).status_code == 422
        assert (
            client.post("/api/v1/instance/access-keys", json={"label": "unknown", "permissions": ["future.permission"]}, headers=headers).status_code
            == 422
        )
...
def test_bearer_delegation_is_strictly_attenuated_and_cannot_redelegate(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        assert client.delete(f"/api/v1/access-keys/{child['parent_id']}", headers=root).status_code == 200
        assert client.get(f"/api/v1/orgs/{org_id}/workspaces", headers=child_headers).status_code == 401
...
def test_expired_access_key_is_reported_as_expired(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        keys = client.get("/api/v1/instance/access-keys", headers=root).json()["data"]
