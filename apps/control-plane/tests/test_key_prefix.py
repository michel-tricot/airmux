...
def test_inference_key_listing_carries_the_token_head(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        minted = _one(c.post(f"/v1/orgs/{org}/workspaces/{workspace}/inference-keys", json={"label": "k"}, headers=headers))
        listed = _one(c.get(f"/v1/orgs/{org}/workspaces/{workspace}/inference-keys", headers=headers))[0]
...
def test_access_key_listing_carries_the_token_head(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        listed = next(k for k in _one(c.get(f"/v1/orgs/{org}/access-keys", headers=headers)) if k["id"] == minted["id"])
...
def test_instance_bound_access_key_uses_the_same_prefix(tmp_path):
    ...
    root = cp.headers()
    with TestClient(cp.app) as c:
        minted = _one(c.post("/v1/instance/access-keys", json={"label": "admin", "permissions": [Permission.organizations_read]}, headers=root))
        listed = next(k for k in _one(c.get("/v1/instance/access-keys", headers=root)) if k["id"] == minted["id"])
...
def test_two_keys_of_a_kind_are_told_apart_by_their_prefix(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        first = _one(c.post(f"/v1/orgs/{org}/access-keys", json=body, headers=headers))
        second = _one(c.post(f"/v1/orgs/{org}/access-keys", json=body, headers=headers))
        listed = {k["id"]: k["prefix"] for k in _one(c.get(f"/v1/orgs/{org}/access-keys", headers=headers))}
