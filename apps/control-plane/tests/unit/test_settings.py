...
def test_database_url_falls_back_without_a_config_file(tmp_path, monkeypatch):
    monkeypatch.setenv("TOKKEEPER_CONFIG", str(tmp_path / "missing.yml"))
    assert database_url() == "postgresql+asyncpg://tokkeeper:tokkeeper@127.0.0.1:5432/tokkeeper"


def test_database_url_comes_from_the_environment(tmp_path, monkeypatch):
    (tmp_path / "tokkeeper.yml").write_text(DATABASE_SECTION, encoding="utf-8")
...
def test_the_shipped_config_loads_with_and_without_a_database_url(tmp_path, monkeypatch):
    ...
    """
    repo_config = Path(__file__).resolve().parents[4] / "tokkeeper.yml"
    monkeypatch.chdir(tmp_path)
    (tmp_path / "tokkeeper.yml").write_text(repo_config.read_text(encoding="utf-8"), encoding="utf-8")
    monkeypatch.delenv("TOKKEEPER_CONFIG", raising=False)
...
def test_the_shipped_config_serves_the_checkout_and_the_stack(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    (tmp_path / "tokkeeper.yml").write_text(repo_config.read_text(encoding="utf-8"), encoding="utf-8")
    (tmp_path / ".tokkeeper").mkdir()
    token = "sk-cp-one-shared-pool-secret-that-is-long-enough"
    (tmp_path / ".tokkeeper" / "dataplane.key").write_text(token, encoding="utf-8")
    monkeypatch.delenv("TOKKEEPER_CONFIG", raising=False)

    for var in ("DATABASE_URL", "TOKKEEPER_CONSOLE_URL", "TOKKEEPER_PUBLIC_SIGNUP"):
    ...
    monkeypatch.setenv("TOKKEEPER_CONSOLE_URL", "http://localhost:3000")
    monkeypatch.setenv("DATABASE_URL", "postgresql+asyncpg://tokkeeper:tokkeeper@postgres:5432/tokkeeper")
    monkeypatch.setenv("TOKKEEPER_PUBLIC_SIGNUP", "true")
    stack = load_settings()
    assert stack.console_url == "http://localhost:3000"
    assert stack.database.url == "postgresql+asyncpg://tokkeeper:tokkeeper@postgres:5432/tokkeeper"
...
def test_container_config_uses_shared_credentials_and_separate_secret_storage(tmp_path, monkeypatch):
    source = Path(__file__).resolve().parents[4] / "deploy" / "docker" / "tokkeeper.yml"
    ...
    monkeypatch.setenv("TOKKEEPER_CONSOLE_URL", "https://console.example.com")
...
def test_a_public_console_does_not_require_an_extra_secret():
    settings = Settings(console_url="https://tokkeeper.example.com")

    assert settings.console_url == "https://tokkeeper.example.com"
...
def test_public_signup_defaults_closed_and_can_be_opened_in_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TOKKEEPER_CONFIG", str(config))
...
def test_shared_migration_config_uses_the_selected_database_url(monkeypatch):
    ...
    monkeypatch.setenv("TOKKEEPER_CONFIG", str(migration_config))
