...
from control_plane.models import AuthSession, PlaygroundSession
...
def test_password_change_revokes_sessions_and_playgrounds_but_preserves_management_keys(tmp_path, credential):
    ...
    with TestClient(cp.app, base_url="https://testserver") as client:
        ...
        assert client.put(path, headers=CSRF).status_code == 200
        ...
        playgrounds = run_in_db(tmp_path, PlaygroundSession.find)
        assert len(playgrounds) == 2
        assert all(playground.revoked for playground in playgrounds)
        assert client.get("/api/v1/bundle/latest", headers=cp.headers(org_id)).json()["data"]["keys"] == []
