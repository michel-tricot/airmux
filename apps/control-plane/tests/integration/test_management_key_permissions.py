...
def test_permission_changes_keep_the_token_and_take_effect_on_the_next_request(tmp_path, scope):
    ...
    with TestClient(cp.app) as client:
        ...
        path = {
            ...
            "org": f"/api/v1/organizations/{org_id}/management-keys",
            "workspace": f"/api/v1/organizations/{org_id}/workspaces/{workspace_id}/management-keys",
        }[scope]
        ...
        resource = f"/api/v1/organizations/{org_id}/workspaces/{workspace_id}"
...
def test_permission_changes_cannot_exceed_actor_or_parent_or_cross_scope(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        key = client.post(
            f"/api/v1/organizations/{org_id}/management-keys", json={"label": "child", "permissions": [Permission.workspaces_read]}, headers=issuer
        ).json()["data"]
...
def test_permission_changes_cannot_exceed_the_key_principals_current_role(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        keys = client.get(f"/api/v1/organizations/{org_id}/management-keys", headers=root).json()["data"]
        ...
        assert client.get(f"/api/v1/organizations/{org_id}", headers=bearer).status_code == 200
...
def test_bearer_cannot_expand_a_key_outside_its_delegation_chain(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        assert response.status_code == 403
        assert client.get(f"/api/v1/organizations/{org_id}/workspaces", headers=target_headers).status_code == 200
