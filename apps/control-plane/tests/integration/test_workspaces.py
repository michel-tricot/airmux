...
def _member(c, cp, org_id, email, role: OrgRole = OrgRole.member):
    ...
    assert c.put(f"/api/v1/organizations/{org_id}/users/{uid}", json={"role": role}, headers=cp.headers(org_id)).status_code == 200
...
def test_org_member_only_sees_joined_workspaces(tmp_path):
    ...
    with TestClient(cp.app, base_url="https://testserver") as c:
        ...
        assert (
            c.put(
                f"/api/v1/organizations/{org_id}/workspaces/{joined}/members/{member_id}",
                json={"role": "viewer"},
                headers=org,
            ).status_code
            == 200
...
def test_workspace_lifecycle_and_creator_auto_enrollment(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert c.get(f"/api/v1/organizations/{o1}/workspaces", headers=member).json()["data"] == []

        created = c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "staging", "slug": "staging"}, headers=member).json()["data"]
        assert created["org_id"] == str(o1)
        assert created["name"] == "staging"
        assert created["slug"] == "staging"
        renamed = c.patch(f"/api/v1/organizations/{o1}/workspaces/{created['id']}", json={"name": "prod"}, headers=member).json()["data"]
        assert renamed["name"] == "prod"
        assert [w["name"] for w in c.get(f"/api/v1/organizations/{o1}/workspaces", headers=member).json()["data"]] == ["prod"]

        members = c.get(f"/api/v1/organizations/{o1}/workspaces/{created['id']}/members", headers=member).json()["data"]
...
def test_workspace_admin_can_list_org_member_candidates_without_org_member_read(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        candidates = c.get(f"/api/v1/organizations/{org_id}/workspaces/{workspace_id}/member-candidates", headers=workspace_admin)
...
def test_workspace_member_lists_each_use_one_resource_query(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        with captured_sql(cp.app) as member_statements:
            members = c.get(f"/api/v1/organizations/{org_id}/workspaces/{workspace_id}/members", headers=workspace_admin)
...
def test_slug_is_unique_within_the_org_and_free_across_orgs(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert (
            c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "Staging", "slug": "staging"}, headers=cp.headers(o1)).status_code == 200
        )
        assert (
            c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "Other staging", "slug": "staging"}, headers=cp.headers(o1)).status_code
            == 409
        )
        assert (
            c.post(f"/api/v1/organizations/{o2}/workspaces", json={"name": "Staging", "slug": "staging"}, headers=cp.headers(o2)).status_code == 200
        )
...
def test_slug_is_create_only(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert c.patch(f"/api/v1/organizations/{o1}/workspaces/{ws}", json={"name": "prod", "slug": "prod"}, headers=org).status_code == 422
        workspace = c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}", headers=org).json()["data"]
...
def test_an_omitted_slug_is_derived_from_the_name(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        def create(name):
            return c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": name}, headers=org).json()["data"]["slug"]
        ...
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/staging-2", headers=org).json()["data"]["name"] == "Staging"
...
def test_a_chosen_slug_is_never_silently_numbered(tmp_path):
    ...
    with TestClient(cp.app) as c:
        o1 = make_org(c, root, "o1")
        org = cp.headers(o1)
        assert c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "Staging"}, headers=org).json()["data"]["slug"] == "staging"
        assert c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "Other", "slug": "staging"}, headers=org).status_code == 409
...
def test_workspace_routes_resolve_by_slug(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/nope", headers=org).status_code == 404

        renamed = c.patch(f"/api/v1/organizations/{o1}/workspaces/staging", json={"name": "prod"}, headers=org).json()["data"]
        assert (renamed["name"], renamed["slug"]) == ("prod", "staging")
        assert c.delete(f"/api/v1/organizations/{o1}/workspaces/staging", headers=org).json()["data"]["id"] == str(ws)
...
def test_slug_shapes_the_api_refuses(tmp_path, slug):
    ...
    with TestClient(cp.app) as c:
        o1 = make_org(c, root, "o1")
        assert c.post(f"/api/v1/organizations/{o1}/workspaces", json={"name": "n", "slug": slug}, headers=cp.headers(o1)).status_code == 422
...
def test_workspace_routes_404_outside_the_org(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{uuid7()}/inference-keys", headers=cp.headers(o1)).status_code == 404
        assert c.get(f"/api/v1/organizations/{o2}/workspaces/{ws}/inference-keys", headers=cp.headers(o2)).status_code == 404
        assert c.patch(f"/api/v1/organizations/{o2}/workspaces/{ws}", json={"name": "x"}, headers=cp.headers(o2)).status_code == 404
        assert c.get(f"/api/v1/organizations/{o2}/workspaces/{ws}/members", headers=cp.headers(o2)).status_code == 404
        assert c.get(f"/api/v1/organizations/{o2}/workspaces", headers=cp.headers(o2)).json()["data"] == []
...
def test_adding_a_non_org_member_is_a_conflict(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert (
            c.put(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{outsider}", json={"role": "member"}, headers=cp.headers(o1)).status_code
            == 409
        )
        assert (
            c.put(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{uuid7()}", json={"role": "member"}, headers=cp.headers(o1)).status_code == 404
        )
...
def test_key_operations_require_workspace_membership(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        admin = cp.headers(o1)
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/inference-keys", headers=admin).status_code == 200
...
def test_membership_lifecycle_within_the_workspace(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/inference-keys", headers=joiner_headers).status_code == 403
        assert c.put(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{joiner}", json={"role": "member"}, headers=creator).status_code == 200
        assert c.put(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{joiner}", json={"role": "member"}, headers=creator).status_code == 200
        joiner_headers = cp.headers_for(o1, joiner, ws)
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/inference-keys", headers=joiner_headers).status_code == 200

        deleted = c.delete(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{joiner}", headers=creator).json()["data"]
        assert deleted["id"] == f"{joiner}/{ws}"
        assert c.delete(f"/api/v1/organizations/{o1}/workspaces/{ws}/members/{joiner}", headers=creator).status_code == 404
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/inference-keys", headers=joiner_headers).status_code == 403
...
def test_org_membership_removal_cascades_out_of_workspaces(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        assert [m["user_id"] for m in c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/members", headers=cp.headers(o1)).json()["data"]] == [uid]

        assert c.delete(f"/api/v1/organizations/{o1}/users/{uid}", headers=cp.headers(o1)).status_code == 200
        assert c.get(f"/api/v1/organizations/{o1}/workspaces/{ws}/members", headers=cp.headers(o1)).json()["data"] == []
