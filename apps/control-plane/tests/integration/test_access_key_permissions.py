...
from control_plane.models import AccessKey, OrgMembership, set_actor
...
def test_permission_changes_keep_the_token_and_take_effect_on_the_next_request(tmp_path, scope):
    ...
    with TestClient(cp.app) as client:
        ...
        path = {
            "instance": "/api/v1/instance/access-keys",
            "org": f"/api/v1/orgs/{org_id}/access-keys",
            "workspace": f"/api/v1/orgs/{org_id}/workspaces/{workspace_id}/access-keys",
        }[scope]
        ...
        changed = client.put(f"/api/v1/access-keys/{key['id']}/permissions", json={"permissions": [Permission.inference_keys_read]}, headers=root)
        ...
        assert (
            client.put(f"/api/v1/access-keys/{key['id']}/permissions", json={"permissions": [Permission.workspaces_read]}, headers=root).status_code
            == 200
        )
        assert client.get(resource, headers=bearer).status_code == 200
...
def test_permission_changes_reject_invalid_sets(tmp_path, permissions):
    ...
    with TestClient(cp.app) as client:
        root = cp.headers()
        key = client.post(
            "/api/v1/instance/access-keys", json={"label": "editable", "permissions": [Permission.workspaces_read]}, headers=root
        ).json()["data"]
        response = client.put(f"/api/v1/access-keys/{key['id']}/permissions", json={"permissions": permissions}, headers=root)
...
def test_permission_changes_cannot_exceed_actor_or_parent_or_cross_scope(tmp_path):
    ...
    with TestClient(cp.app) as client:
        root = cp.headers()
        org_id = make_org(client, root)
        issuer = cp.headers(org_id, permissions=[Permission.access_keys_issue, Permission.workspaces_read, Permission.inference_keys_read])
        key = client.post(
            f"/api/v1/orgs/{org_id}/access-keys", json={"label": "child", "permissions": [Permission.workspaces_read]}, headers=issuer
        ).json()["data"]
        path = f"/api/v1/access-keys/{key['id']}/permissions"
        ...
        assert client.put(path, json={"permissions": [Permission.access_keys_issue]}, headers=root).status_code == 403
        reader = cp.headers(org_id, permissions=[Permission.access_keys_read])
...
def test_inactive_keys_cannot_be_edited(tmp_path, status):
    ...
    with TestClient(cp.app) as client:
        root = cp.headers()
        key = client.post(
            "/api/v1/instance/access-keys", json={"label": "inactive", "permissions": [Permission.workspaces_read]}, headers=root
        ).json()["data"]

        async def deactivate():
            stored = await AccessKey.find_by_id(key["id"])
        ...
        assert (
            client.put(
                f"/api/v1/access-keys/{key['id']}/permissions", json={"permissions": [Permission.inference_keys_read]}, headers=root
            ).status_code
            == 409
        )
...
def test_reducing_parent_permissions_limits_existing_child_tokens(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        response = client.put(
            f"/api/v1/access-keys/{child['parent_id']}/permissions", json={"permissions": [Permission.inference_keys_read]}, headers=root
        )
...
def test_permission_changes_cannot_exceed_the_key_principals_current_role(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        response = client.put(f"/api/v1/access-keys/{key['id']}/permissions", json={"permissions": [Permission.members_manage]}, headers=root)
...
def test_permission_updates_reject_other_fields_and_missing_permissions(tmp_path):
    ...
    with TestClient(cp.app) as client:
        root = cp.headers()
        key = client.post(
            "/api/v1/instance/access-keys", json={"label": "unchanged", "permissions": [Permission.workspaces_read]}, headers=root
        ).json()["data"]
        for body in (
            {},
            ...
            assert client.put(f"/api/v1/access-keys/{key['id']}/permissions", json=body, headers=root).status_code == 422
...
def test_bearer_cannot_expand_a_key_outside_its_delegation_chain(tmp_path):
    ...
    with TestClient(cp.app) as client:
        ...
        keys = client.get(f"/api/v1/orgs/{org_id}/access-keys", headers=root).json()["data"]
        target = next(key for key in keys if key["permissions"] == [Permission.workspaces_read])
        editor = cp.headers(org_id, permissions=[Permission.access_keys_issue, Permission.workspaces_read, Permission.inference_keys_read])
        response = client.put(
            f"/api/v1/access-keys/{target['id']}/permissions", json={"permissions": [Permission.inference_keys_read]}, headers=editor
        )
        assert response.status_code == 403
