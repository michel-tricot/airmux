...
def test_get_org_by_id(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        fetched = c.get(f"/api/v1/orgs/{org}", headers=root)
        assert fetched.status_code == 200, fetched.text
        assert fetched.json()["data"] == next(o for o in c.get("/api/v1/orgs", headers=root).json()["data"] if o["id"] == str(org))
        assert c.get(f"/api/v1/orgs/{uuid7()}", headers=root).status_code == 404
...
def test_get_user_by_id_carries_their_memberships(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        c.put(f"/api/v1/orgs/{org}/users/{user.id}", json={"role": "member"}, headers=cp.headers(org))
...
def test_get_workspace_by_id_stays_inside_the_org(tmp_path):
    ...
    with TestClient(cp.app) as c:
        ...
        fetched = c.get(f"/api/v1/orgs/{org}/workspaces/{workspace}", headers=cp.headers(org))
        assert fetched.status_code == 200, fetched.text
        assert fetched.json()["data"]["name"] == "staging"

        assert c.get(f"/api/v1/orgs/{other}/workspaces/{workspace}", headers=cp.headers(other)).status_code == 404
        assert c.get(f"/api/v1/orgs/{org}/workspaces/{uuid7()}", headers=cp.headers(org)).status_code == 404
