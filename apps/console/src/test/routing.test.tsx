...
// `/org/workspaces/:workspaceId` route capturing `/keys`).
...
describe('workspace section deep links', () => {
  it.each(SECTIONS)('renders the right page for /org/workspaces/:id$suffix', async ({ suffix, heading }) => {
    renderAt(`/org/workspaces/${WS.id}${suffix}`);
...
describe('default workspace selection', () => {
  it('redirects /org to the first workspace when none was selected before', async () => {
    renderAt('/org');
    await waitFor(() => {
      expect(window.location.pathname).toBe(`/org/workspaces/${WORKSPACES[0].id}`);
    });
  ...
  it('redirects /org to the last-selected workspace', async () => {
    ...
    renderAt('/org');
    await waitFor(() => {
      expect(window.location.pathname).toBe(`/org/workspaces/${WORKSPACES[1].id}`);
    });
  ...
  it('remembers the workspace visited via a deep link', async () => {
    renderAt(`/org/workspaces/${WORKSPACES[1].id}/keys`);
...
describe('workspace switching keeps the active section', () => {
  it.each(SECTIONS)('stays on $suffix when switching workspaces', async ({ suffix, heading }) => {
    ...
    renderAt(`/org/workspaces/${WORKSPACES[0].id}${suffix}`);
    ...
    await waitFor(() => {
      expect(window.location.pathname).toBe(`/org/workspaces/${WORKSPACES[1].id}${suffix}`);
    });
    ...
    const active = document.querySelector(`a[href="/org/workspaces/${WORKSPACES[1].id}${suffix}"]`);
