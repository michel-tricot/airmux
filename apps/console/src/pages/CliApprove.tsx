...
function lookupError(error: unknown): string {
  if (error instanceof ApiError) {
    if (error.status === 404) return 'No pending login with this code. Check your terminal, or run airmux login again.';
    if (error.status === 410) return 'This login request expired. Run airmux login again.';
      ...
      <Card className="w-full max-w-md p-8 shadow-xl border-border/50 space-y-6">
        <div className="flex flex-col items-center">
          ...
          <p className="text-muted-foreground text-sm mt-1 text-center">
            Continue only if you just ran <code>airmux login</code>.
          </p>
        </div>
