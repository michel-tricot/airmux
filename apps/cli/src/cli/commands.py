...
from cli.common import CONNECTION, GETTING_STARTED, GOODIES, RESOURCES, SERVICES, app, console
...
CATEGORY_ORDER = [GETTING_STARTED, CONNECTION, GOODIES, SERVICES, RESOURCES]
UNCATEGORISED = "Other"
...
def commands(fmt: FormatOption = OutputFormat.table) -> None:
    ...
    found = [row for row in walk(root, "airmux", UNCATEGORISED) if row["command"] != "airmux commands"]
