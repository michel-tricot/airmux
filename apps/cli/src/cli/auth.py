...
def example_model(catalog: dict, seeded: set[str]) -> str:
    ...
    The catalog is the taxonomy endpoint's wire shape: models name their provider by id, and
    the provider entries carry the names credentials are seeded under."""
    provider_ids = {provider["id"] for provider in catalog.get("providers", []) if provider["name"] in seeded}
    models = catalog.get("models", [])
    routable = [model for model in models if model["provider_id"] in provider_ids] or models
    if not routable:
        return "openai/gpt-4o-mini"
    return min(routable, key=lambda model: (model.get("input_price_per_mtok") or 0.0, model["name"]))["name"]
...
def quickstart(  # noqa: PLR0913, PLR0917 flags are the command's interface
    ...
    with httpx.Client(base_url=url, timeout=10.0, headers=CSRF) as c:
        ...
        example = example_model(catalog.json()["data"] if catalog.is_success else {}, {result.provider for result in results if not result.error})
