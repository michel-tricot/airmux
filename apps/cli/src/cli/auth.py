...
CSRF = {"X-Requested-With": "airmux-cli"}
...
def quickstart(  # noqa: PLR0913, PLR0917 flags are the command's interface
    ...
    console.print("[bold]airmux quickstart[/bold]")
    with httpx.Client(base_url=control_plane_url, timeout=10.0, headers=CSRF) as c:
        ...
        console.print(Panel(key.token, title="AIRMUX_INFERENCE_KEY", border_style="cyan", expand=False))
    ...
    if model is None:
        ...
        console.print("Enable or add a provider key, then run [bold]airmux quickstart[/bold] again.")
        raise typer.Exit(1)
    error = verify_gateway(gateway_url, key.token, model)
    if error:
        console.print(f"\n[red]Setup is incomplete: the gateway request failed: {error}[/red]")
        console.print("Run [bold]airmux doctor[/bold] after resolving the reported gateway issue.")
        raise typer.Exit(1)
    ...
    with httpx.Client(base_url=control_plane_url, timeout=10.0) as c:
        ...
        while time.monotonic() < deadline:
            ...
            if poll.status_code == HTTP_GONE:
                console.print("[red]Login expired before it was approved. Run [bold]airmux login[/bold] again.[/red]")
                raise typer.Exit(1)
            ensure_ok(poll)
            ...
            if done.status == "complete":
                ...
                return
    console.print("[red]Login timed out. Run [bold]airmux login[/bold] again.[/red]")
    raise typer.Exit(1)
