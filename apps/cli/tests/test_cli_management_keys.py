...
from api_models import ManagementKeyCreatedOut, Scope
...
def test_instance_flag_overrides_the_active_org_for_management_key_creation(monkeypatch):
    submitted = {}
    monkeypatch.setattr(resources, "access_client", lambda _url: Client(submitted))

    result = runner.invoke(app, ["management-keys", "create", "--label", "ci", "--permission", "bundles.read", "--instance"])
