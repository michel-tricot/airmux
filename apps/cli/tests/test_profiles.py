...
def test_profile_config_rejects_invalid_known_fields(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TOKKEEPER_CLI_CONFIG", str(path))
...
def test_cli_reports_invalid_config_without_a_traceback_or_secrets(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["tokkeeper", "status"])
...
def test_url_profile_updates_the_same_name_on_the_same_deployment(tmp_path, monkeypatch):
    ...
    replacement = {**first, "control_plane_url": "https://tokkeeper.example.com/", "token": "new"}
...
def test_url_profile_keeps_the_same_name_on_different_deployments(tmp_path, monkeypatch):
    ...
    fly = {"control_plane_url": "https://tokkeeper-example.fly.dev", "scope": "org", "org_id": "org-1", "org_name": "michel", "token": "fly"}

    assert upsert_url_profile("michel", Profile.model_validate(local)) == "michel"
    assert upsert_url_profile("michel", Profile.model_validate(fly)) == "michel@tokkeeper-example.fly.dev"
    set_active("michel")
    assert upsert_url_profile("michel", Profile.model_validate({**fly, "token": "fly-new"})) == "michel@tokkeeper-example.fly.dev"

    config = load_config()
    assert config.active == "michel@tokkeeper-example.fly.dev"
    assert config.profiles["michel"].token == "local"
    assert config.profiles["michel@tokkeeper-example.fly.dev"].token == "fly-new"
...
def test_instance_profile_does_not_replace_an_organization_named_instance(tmp_path, monkeypatch):
    ...
    assert upsert_url_profile("instance", Profile.model_validate(instance)) == "instance@tokkeeper.example.com"
