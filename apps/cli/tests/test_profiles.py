...
def stored_profile(**values: object) -> Profile:
...
def test_profile_round_trip_and_permissions(tmp_path, monkeypatch):
    ...
    upsert_profile("acme", stored_profile(control_plane_url="http://cp:8000", org_id="o1", org_name="acme", token="sk-cp-x"))
    upsert_profile("beta", stored_profile(control_plane_url="http://cp:8000", org_id="o2", org_name="beta", token="sk-cp-y"))
...
def test_upsert_preserves_unknown_settings(tmp_path, monkeypatch):
    ...
    upsert_profile("acme", stored_profile(token="t"))
    assert load_config().model_extra == {"settings": {"color": "never"}}
...
def test_removing_the_active_profile_selects_the_next_profile(tmp_path, monkeypatch):
    monkeypatch.setenv("GW_CLI_CONFIG", str(tmp_path / "config.toml"))
    upsert_profile("acme", stored_profile(token="first"))
    upsert_profile("beta", stored_profile(token="second"))
