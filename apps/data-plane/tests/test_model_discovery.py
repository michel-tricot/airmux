...
def test_openai_sdk_lists_and_retrieves_gateway_models(http_mock, api_key, dp_app):
...
def test_discovery_requires_an_active_inference_key(http_mock, dp_app, path, token):
    mock_control_plane(http_mock)
    with TestClient(dp_app) as client:
...
def test_sdk_discovery_errors_are_typed(http_mock, dp_app):
    mock_control_plane(http_mock)
    with TestClient(dp_app) as client:
...
@pytest.mark.parametrize("path", PATHS)
def test_discovery_rejects_expired_keys(http_mock, tmp_path, path):
    token, key = make_key()
    bundle = make_bundle(
...
@pytest.mark.parametrize("path", PATHS)
def test_discovery_returns_503_without_a_bundle(http_mock, tmp_path, path):
    mock_control_plane(http_mock)
    with TestClient(create_app(make_config(tmp_path, "devnull"))) as client:
...
def test_gateway_model_ids_can_contain_slashes_and_do_not_expose_upstream_details(http_mock, tmp_path):
    token, key = make_key()
...
def test_discovery_hides_models_blocked_by_model_policies(http_mock, tmp_path, action, target):
...
def test_discovery_preserves_key_workspace_and_request_match_scoping(http_mock, tmp_path, options):
...
def test_models_without_credentials_in_the_callers_scope_are_hidden(http_mock, tmp_path, credentials):
...
def test_discovery_includes_models_with_credentials_allowed_by_policy(http_mock, tmp_path, scope):
    credentials = {
...
def test_discovery_defers_inference_only_actions(http_mock, tmp_path, action):
...
def test_model_matching_policies_only_hide_the_selected_model(http_mock, tmp_path):
...
def test_discovery_uses_the_callers_org_and_the_current_bundle(http_mock, tmp_path):
    other_org = uuid7()
