...
def _poller(tmp_path, holder, private_key, http_client) -> BundlePoller:
    link, bundle_config = _source(tmp_path)
    bundle_config = bundle_config.model_copy(update={"verify_key": private_key.public_key()})
    return BundlePoller(link, bundle_config, holder, http_client)
...
async def test_poll_swaps_and_persists(tmp_path, http_client):
    ...
    holder = BundleHolder()
    await _poller(tmp_path, holder, private_key, http_client).once()
...
async def test_poll_same_bundle_is_a_noop(tmp_path, http_client):
    ...
    respx.get("http://cp.test/v1/bundle/latest").mock(return_value=httpx.Response(200, content=enveloped(signed)))
    holder = BundleHolder()
    poller = _poller(tmp_path, holder, private_key, http_client)
    await poller.once()
