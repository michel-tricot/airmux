...
def test_an_open_schema_forwards_extras_without_an_adjustment():
    req, adjustments = reconcile(
        _request(max_output_tokens=MODEL.max_output_tokens, frequency_penalty=0.5, top_k=40), MODEL, compile_profile(PROVIDER)
    )
...
def test_a_closed_schema_drops_undeclared_extras_with_the_reason():
    req, adjustments = reconcile(_request(max_output_tokens=MODEL.max_output_tokens, top_k=40), MODEL, compile_profile(_closed()))
    assert req.extra == {}
...
def test_a_closed_schema_forwards_its_declared_params():
    req, adjustments = reconcile(
        _request(max_output_tokens=MODEL.max_output_tokens, top_k=40, min_p=0.1), MODEL, compile_profile(_closed(accepted_params=["top_k"]))
    )
...
def test_n_is_held_by_the_gateway_everywhere():
    ...
    req, adjustments = reconcile(_request(max_output_tokens=MODEL.max_output_tokens, n=3), MODEL, compile_profile(PROVIDER))
    assert req.extra == {}
    assert [(a.param, a.action) for a in adjustments] == [("n", "dropped")]


def test_without_alias_knowledge_an_unknown_spelling_forwards():
    """The gateway cannot know max_completion_tokens is a spelling of max_output_tokens unless the
    provider profile says so; without that fact it is an ordinary extra on an open schema."""
    req, adjustments = reconcile(_request(max_output_tokens=100, max_completion_tokens=999), MODEL, compile_profile(PROVIDER))
    assert req.extra == {"max_completion_tokens": 999}
    assert adjustments == []


def test_an_extra_colliding_with_an_alias_spelling_drops():
    """The provider spells max_output_tokens as max_completion_tokens; an extra by that name would collide
    ...
    assert "max_output_tokens" in adjustments[0].detail


def test_the_clamp_still_reports():
    model = MODEL.model_copy(update={"max_output_tokens": 50})
    req, adjustments = reconcile(_request(max_output_tokens=100), model, compile_profile(PROVIDER))
    assert req.max_output_tokens == 50
    assert [(a.param, a.action, a.source) for a in adjustments] == [("max_output_tokens", "clamped", "model")]


@pytest.mark.parametrize(
    ("policy_limit", "model_limit", "expected", "source"),
    [
        (2000, 4000, 2000, "policy"),
        (4000, 2000, 2000, "model"),
        (2000, 2000, 2000, "policy_and_model"),
        (None, 2000, 2000, "model"),
    ],
)
def test_an_omitted_caller_limit_defaults_to_the_tightest_ceiling(policy_limit, model_limit, expected, source):
    model = MODEL.model_copy(update={"max_output_tokens": model_limit})

    req, adjustments = reconcile(_request(), model, compile_profile(PROVIDER), policy_limit)

    assert req.max_output_tokens == expected
    assert [(a.param, a.action, a.source) for a in adjustments] == [("max_output_tokens", "defaulted", source)]


def test_an_omitted_caller_limit_stays_unspecified_without_a_ceiling():
    model = MODEL.model_copy(update={"max_output_tokens": None})

    req, adjustments = reconcile(_request(), model, compile_profile(PROVIDER), None)
...
def test_an_unsupported_model_parameter_is_dropped_with_an_adjustment():
    ...
    req, adjustments = reconcile(_request(max_output_tokens=MODEL.max_output_tokens, temperature=0.7), model, compile_profile(PROVIDER))
