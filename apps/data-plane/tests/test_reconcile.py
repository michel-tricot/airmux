...
def test_an_open_schema_forwards_extras_without_an_adjustment():
    ...
    req, adjustments = reconcile(_request(frequency_penalty=0.5, top_k=40), MODEL, PROVIDER)
    assert req.extra == {"frequency_penalty": 0.5, "top_k": 40}
    assert adjustments == []


def test_a_closed_schema_drops_undeclared_extras_with_the_reason():
    req, adjustments = reconcile(_request(top_k=40), MODEL, _closed())
    assert req.extra == {}
...
def test_a_closed_schema_forwards_its_declared_params():
    req, adjustments = reconcile(_request(top_k=40, min_p=0.1), MODEL, _closed(accepted_params=["top_k"]))
...
def test_n_is_held_by_the_gateway_everywhere():
    ...
    req, adjustments = reconcile(_request(n=3), MODEL, PROVIDER)
    assert req.extra == {}
...
def test_without_alias_knowledge_an_unknown_spelling_forwards():
    ...
    req, adjustments = reconcile(_request(max_tokens=100, max_completion_tokens=999), MODEL, PROVIDER)
...
def test_the_clamp_still_reports():
    ...
    req, adjustments = reconcile(_request(max_tokens=100), model, PROVIDER)
