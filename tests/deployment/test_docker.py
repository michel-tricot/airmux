...
def test_onboarding_inference_streaming_and_persistence(deployment, tmp_path):
    ...
    gateway = gateways[0]
    provider = "deployment"
    assert payload(client.get("/api/v1/instance/oss/claim")) == {"claimed": False, "public_signup": True}
    ...
    assert payload(client.get("/api/v1/instance/oss/claim")) == {"claimed": True, "public_signup": True}
