...
TOOLS: dict[Dialect, list[dict[str, object]]] = {
    "openai_chat_completions": [{"type": "function", "function": WEATHER}],
...
def test_system_prompt_reaches_the_provider_in_its_designated_field(gateway: Gateway, dialect: Dialect, family: Family):
    ...
    caller_prompts: dict[Dialect, dict[str, object]] = {
        "openai_chat_completions": {"messages": system_messages},
...
def test_inline_content_preserves_type_media_data_and_order(gateway: Gateway, dialect: Dialect, family: Family, attachment: Attachment):
    ...
    expected_filename = {"openai_chat_completions": "test.pdf", "openai_responses": "test.pdf", "anthropic": "document.pdf"}[dialect]
    ...
    caller_text_type = {"openai_chat_completions": "text", "openai_responses": "input_text", "anthropic": "text"}[dialect]
...
def test_structured_output_schema_reaches_the_provider(gateway: Gateway, dialect: Dialect, family: Family):
    caller_formats: dict[Dialect, dict[str, object]] = {
        "openai_chat_completions": {"response_format": {"type": "json_schema", "json_schema": {"name": "city", "schema": CITY_SCHEMA}}},
    ...
    expected_metadata = {
        "openai_chat_completions": {"name": "city"},
...
def test_provider_aliases_cannot_bypass_the_canonical_parameter_boundary(gateway: Gateway, family: Family, alias_input: bool):
    ...
    gateway.start()
    response = gateway.request(body={**request_body("openai_chat_completions"), "provider_temperature" if alias_input else "temperature": 0.5})
