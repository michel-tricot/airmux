...
def test_management_key_is_self_owned_shown_once_and_revoked(stack: Stack) -> None:
    with running_console(stack) as console:
        ...
        dialog.get_by_role("checkbox", name="catalog.read", exact=True).check()
        ...
        me = console.context.request.get(f"{console.url}/api/v1/auth/me", headers={"X-Requested-With": "fetch"}).json()["data"]
