...
def test_enrollment_hides_foreign_invitation_metadata_over_http(stack):
    ...
    with httpx.Client(base_url=stack.cp_url, headers={"X-Requested-With": "XMLHttpRequest"}) as browser:
        ...
        visible_org = browser.post("/api/v1/orgs", json={"name": "Visible invitations"})
        visible_org.raise_for_status()
        org_id = visible_org.json()["data"]["id"]
        private_org = browser.post("/api/v1/orgs", json={"name": "Private invitations"})
        private_org.raise_for_status()
        private_id = private_org.json()["data"]["id"]
        workspace = browser.post(f"/api/v1/orgs/{org_id}/workspaces", json={"name": "Selected"})
        workspace.raise_for_status()
        workspace_id = workspace.json()["data"]["id"]
        body = {"email": ADMIN_EMAIL, "org_role": "member"}
        browser.post(f"/api/v1/orgs/{org_id}/invitations", json={**body, "workspace_id": workspace_id, "workspace_role": "viewer"}).raise_for_status()
        browser.post(f"/api/v1/orgs/{private_id}/invitations", json=body).raise_for_status()
        browser.put(f"/api/v1/orgs/{org_id}/users/{user_id}", json={"role": "owner"}).raise_for_status()
        ...
        for prefix in (f"/api/v1/orgs/{org_id}", f"/api/v1/orgs/{org_id}/workspaces/{workspace_id}"):
