...
if [[ ! -f .tokkeeper/dataplane.key ]]; then
  uv run tokkeepercp bootstrap-keygen
fi

if ! uv run tokkeepercp migrate; then
  reset_database
  uv run tokkeepercp migrate
fi

uv run tokkeepercp taxonomy --file taxonomy/taxonomy.yml
...
exec uv run tokkeepercp serve --dev --host 127.0.0.1 --port 8101
