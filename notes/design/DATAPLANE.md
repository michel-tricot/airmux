...
| `GET /healthz` | `200` when this worker has accepted a configuration and can admit metering work, otherwise `503` |
| `GET /metrics` | Prometheus metrics on a standalone plane's internal listener |

The health routes require no authentication. Readiness means that a configuration has been admitted and the
...
- `healthz` reports accepted configuration and local worker availability only
