...
The Protect Main ruleset requires the PR CI `required` and Security `dependency-security` checks. PR CI runs the same five jobs for draft and ready pull requests. Main CI calls that workflow and then runs the broader candidate jobs after merge. Main CI's gate has a distinct display name because GitHub can treat duplicate job names across workflows as ambiguous required checks. It requires the reusable fast job and every broader job. Do not add path filters that can skip a required check.
...
The shared `package` job builds one source distribution and rebuilds its wheel from that source distribution. It uploads the candidate with `SHA256SUMS` and exposes the artifact ID as a reusable-workflow output. Main CI waits for all fast checks before starting gateway, full-stack, browser, and Docker validation against that artifact. The Docker job publishes the validated main image under a commit-specific GHCR tag and uploads its immutable digest reference.
